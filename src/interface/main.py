...
def main():
    ...
    try:
        ...
        # *** Seção da antiga coluna esquerda ***
        st.subheader("Dados do circuito")

        with st.container():  # Removi height=350
            plot_title_graph_left = st.empty()  # Placeholder for the graph title
        ...
        plot_title_graph_left.subheader(f"{option}")

        # Calculate and plot the open loop
        K, tau_estimado, theta_estimado = estimate_pid_values(tempo, degrau, potencia_motor)

        if option == "Dataset":
            fig_left = plot_dataset_graph_plotly(tempo, degrau, potencia_motor, "Dataset")
            ...
            fig_left = plot_graph_open_loop_plotly(
            ...
            fig_left = plot_graph_closed_loop_plotly(
                tempo_closed, saida_closed, tempo, degrau, "Resposta em Malha Fechada"
            )

        # Display the graph in the placeholder
        plot_left_placeholder.plotly_chart(fig_left, use_container_width=True)

        # *** Seção da antiga coluna direita ***
        st.subheader("Gráfico do controlador PID")

        # Container for the graph with a fixed height
        with st.container():  # Removi height=350
            plot_title_graph_right = st.empty()  # Placeholder for the graph title
        ...
        plot_title_graph_right.subheader(f"{option2}")
        ...
        if option2 == "CHR":
            ...
            fig_right = plot_graph_pid_plotly(tempo_chr, saida_chr, tempo, option2, degrau)
        elif option2 == "ITAE":
            tempo_itae, saida_itae = ITAE(k, tau_i, theta_d, amplitude_degrau, tempo)
            fig_right = plot_graph_pid_plotly(tempo_itae, saida_itae, tempo, option2, degrau)

        # Update the graph in the placeholder
        plot_right_placeholder.plotly_chart(fig_right, use_container_width=True)
